...
var q = 'INSERT INTO bookings (party_size, save_date, table_time, restaurant_id) VALUES (?, ?, ?, ?)';
for (var i = 0; i < sample.data.length; i++) { //Insert 700 random datasets into table bookings. (REFER TO SCHEMA IF NEEDED).
	connection.query(q, [sample.data[i].partySize, sample.data[i].date, time[randomRest(0,46)], randomRest(1,100)], function(err, results) {
		if (err) {
